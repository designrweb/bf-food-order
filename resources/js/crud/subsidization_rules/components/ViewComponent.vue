        ...
        <div class="card">
            <div class="card-header">
                <h4 class="card-sub-title">
                    Subventionierte Menükategorien
                </h4>
            </div>
            <div class="text-center" v-if="isPageBusy">
                <spinner-component></spinner-component>
            ...
            <div v-else>
                <ul class="list-group list-group-flush" v-if="itemData.subsidized_menu_categories">
                    <li class="list-group-item d-flex justify-content-between"
                        v-for="(subsidizedMenuCategories, index) in itemData.subsidized_menu_categories"
                        :key="index">
                        <span>{{ subsidizedMenuCategories.menu_category.name }}</span>
                        <span>{{ subsidizedMenuCategories.percent }}%</span>
                    </li>
                </ul>
                <div v-else class="container mt-3 mb-3">
                    <div class="text-center text-gray">
                        <h2 class="card-text no-results">Es sind keine Aufzeichnungen zu zeigen</h2>
                    </div>
                </div>
            </div>
        </div>
